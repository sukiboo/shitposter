...
def test_step_sets_state(run_ctx):
    ...
    with (
        patch(
            "shitposter.providers.text_to_int.OpenAITextToIntProvider.__init__", return_value=None
        ),
        patch("shitposter.providers.text_to_int.OpenAITextToIntProvider.generate", return_value=2),
        patch(
